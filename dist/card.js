...
class $b161f025c07cf354$export$7fe46a8978a1b23d extends (0, $ab210b2da7b39b9d$export$3f2f9f5909897157) {
    ...
    setLighting() {
        ...
        Object.entries(entities).forEach(([key, value])=>{
            if (value.labels.includes("lighting")) {
                ...
                if (labels.length === 0) this._lighting[floorId][entityId] = [
                    light
                ];
                else {
                    const label = labels[0];
                    if (label in this._lighting[floorId]) this._lighting[floorId][label].push(light);
                    else this._lighting[floorId][label] = [
                        light
                    ];
                }
            }
    ...
    prettyFloor(floor) {
        ...
        if (output === "First" || output === "Second") output = output + " Floor";
        return output;
    }
    getOnTot(floor) {
